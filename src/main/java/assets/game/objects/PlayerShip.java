...
public final class PlayerShip extends Ship {
    ...
    public void tick(float deltaTime) {
        ...
        double desiredRot =
                Math.toDegrees(
                        Math.atan2(
                                Mouse.getX() - (pos.x - Globals.camera.x),
                                (Globals.HEIGHT - Mouse.getY()) - (pos.y - Globals.camera.y)));
        ...
        //        if(isAccelerating) Game.add(new EngineParticle(x -
        // Math.cos(Math.toRadians(rotation)) * 6.5, y - Math.sin(Math.toRadians(rotation)) * 6.5,
        // -Math.cos(Math.toRadians(rotation))*1.5, -Math.sin(Math.toRadians(rotation))*1.5, new
        // Color(200, 200, 200, 145)));
        //        else Game.add(new EngineParticle(x - Math.cos(Math.toRadians(rotation)) * 12, y -
        // Math.sin(Math.toRadians(rotation)) * 12, Color.DARK_GRAY));
        if (frames % 3 == 0) {
            if (Globals.isMulti()) {
