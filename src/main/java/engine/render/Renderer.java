...
public class Renderer implements Runnable {
    ...
    public void run() {
        ...
        while (!Display.isCloseRequested()) {
            ...
            if (isInterpolating) {
                interpolation =
                        Math.min(
                                1.0f,
                                (float)
                                        ((now - Globals.TICKER.lastTickTime)
                                                / (TimeUnit.SECONDS.toMillis(1)
                                                        / Globals.TICKER.targetTPS)));
            } else {
                interpolation = 1;
            }
            clearRender();
            ...
            screenShader.loadTransformationMatrix(
                    Util.createSpriteTransformationMatrix(
                            0, 0, 0, Globals.WIDTH, Globals.HEIGHT, 0));
            ...
            // System.out.println("render " + renders + " done in " + renderTime + "ms.  FPS: " +
            // Math.round(1/(renderTime/1_000f)));
            Display.setTitle(
                    "Squadron 4  -  TPS: "
                            + Globals.TICKER.getTPS()
                            + "  -  Sprites: "
                            + Globals.renderObjects.size());
        }
    ...
    public void renderSprite(Sprite toRender, SpriteShader shader) {
        ...
        if (toRender instanceof Interpolatable) {
            ...
            tMatrix =
                    Util.createSpriteTransformationMatrix(
                            toRender.getRenderX() + (i.getDeltaX() * interpolation),
                            toRender.getRenderY() + (i.getDeltaY() * interpolation),
                            toRender.getRotation(),
                            toRender.getWidth(),
                            toRender.getHeight(),
                            toRender.getPriority());
        } else {
            tMatrix =
                    Util.createSpriteTransformationMatrix(
                            toRender.getRenderX(),
