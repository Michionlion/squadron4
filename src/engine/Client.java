...
//            System.out.println("attempting remove on ship: " + msg.substring(5) + " with org msg: " + msg);
...
//        System.out.println(info.length + " length array consists of:" + info[0] + "," + info[1] + "," + info[2] + "," + info[3] + "," + info[4] + "," + info[5] + "," + info[6]);
        try {
            if (info.length < 7) {
                throw new UnsupportedOperationException("Not enough arguments for projectile creation");
            }
            name = info[0];
...
//        System.out.println("creating new projectile with: " + spawnX + " " + spawnY + " " + deltaX + " " + deltaY + " " + angle + " " + type);
    ...
    public void sendProj(double spawnX, double spawnY, double deltaX, double deltaY, double angle, int type) {
//        System.out.println("sending: " + spawnX + Client.sepChar + spawnY + Client.sepChar + deltaX + Client.sepChar + deltaY + Client.sepChar + angle + Client.sepChar + type);
        sendMSG("/proj", spawnX + Client.sepChar + spawnY + Client.sepChar + deltaX + Client.sepChar + deltaY + Client.sepChar + angle + Client.sepChar + type);
    }

    public void sendProj(Projectile p) {
        sendMSG("/proj", p.getX() + Client.sepChar + p.getY() + Client.sepChar + p.getDeltaX() + Client.sepChar + p.getDeltaY() + Client.sepChar + p.getRotation() + Client.sepChar + p.getType());
    }
